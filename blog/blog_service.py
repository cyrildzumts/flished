...
def get_new_post_comments(post_id,data) :
    ...
    if queryset.exists():
        ...
        comments = queryset.annotate(username=Concat('author__first_name', Value(" ") , 'author__last_name', output_field=CharField()), date=Func(F('created_at'),Value('DD MM YYYY HH:MM'),output_field=CharField())).values('username', 'post_id', 'comment', 'date')
